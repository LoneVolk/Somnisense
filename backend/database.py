...
def init_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS sleep_records (
            ...
            awakenings_json   TEXT,
            created_at        TIMESTAMP DEFAULT NOW(),
            UNIQUE(user_id, date)
        )
    """)

    # Миграция: добавить awakenings_json если её ещё нет (для старых БД)
    cur.execute("""
        ALTER TABLE sleep_records
        ADD COLUMN IF NOT EXISTS awakenings_json TEXT
    """)
...
def save_sleep_record(record: dict, user_id: str = "default") -> int:
    # awakenings_json — опциональное поле, дефолт None
    record.setdefault("awakenings_json", None)
    conn = get_connection()
    cur = conn.cursor()
    try:
        cur.execute("""
            INSERT INTO sleep_records (
                ...
                spo2_avg, spo2_min, sleep_score, awakenings_count, source, awakenings_json
            ) VALUES (
                ...
                %(spo2_avg)s, %(spo2_min)s, %(sleep_score)s, %(awakenings_count)s, %(source)s, %(awakenings_json)s
            )
            ON CONFLICT (user_id, date) DO UPDATE SET
                ...
                awakenings_json  = COALESCE(EXCLUDED.awakenings_json, sleep_records.awakenings_json)
            RETURNING id
